import { potions, recipes } from "../data.js";
import { selectedIngredients } from "./addIngredient.js";
export const brewPotion = () => {
    if (selectedIngredients.length === 2 &&
        selectedIngredients[0] &&
        selectedIngredients[1]) {
        for (const recipie of recipes) {
            if (recipie.ingredients.includes(selectedIngredients[0]) &&
...
const foundPotion = (recipie) => {
    ...
    if (potion) {
        ...
        const correctImage = images.find((img) => img.id === potion.id);
        if (correctImage) {
